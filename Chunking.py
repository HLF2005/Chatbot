...
def fixed_size_overlapping(text, chunk_size, overlapping_size=0):
   return [text[i:i+chunk_size] for i in range(0,len(text), chunk_size - overlapping_size)]

def recursive_overlapping_chunking(text, separators, chunk_size=300):
    if len(text) <= chunk_size:
        return [text]
    current_sep = separators[0] if separators else None
    if current_sep and current_sep in text:
